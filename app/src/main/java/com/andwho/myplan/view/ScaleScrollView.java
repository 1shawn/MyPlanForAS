 ...
 */
public class  ScaleScrollView extends ScaleBase<ScrollView> {
    private static final String TAG = ScaleScrollView.class.getSimpleName();
    ...
    public ScaleScrollView(Context context, AttributeSet attrs) {
        ...
        ((InternalScrollView) mRootView).setOnScrollViewChangedListener(new OnScrollViewChangedListener() {
            ...
            public void onInternalScrollChanged(int left, int top, int oldLeft, int oldTop) {
                if (isPullToScaleEnabled() && isParallax()) {
    ...
    @Override
    protected void pullHeaderToScale(int newScrollValue) {
        ...
        if (isCustomHeaderHeight) {
            ViewGroup.LayoutParams ScaleLayoutParams = mScaleView.getLayoutParams();
            ScaleLayoutParams.height = Math.abs(newScrollValue) + mHeaderHeight;
            mScaleView.setLayoutParams(ScaleLayoutParams);
        }
    }
    ...
    private void updateHeaderView() {
        if (mHeaderContainer != null) {
            ...
            if (mScaleView != null) {
                mHeaderContainer.addView(mScaleView);
            }
    ...
    public void handleStyledAttributes(TypedArray a) {
        ...
        int contentViewResId = a.getResourceId(R.styleable.PullToScaleView_contentView, 0);
    ...
    protected void onLayout(boolean paramBoolean, int paramInt1, int paramInt2,
        ...
        if (mHeaderHeight == 0 && mScaleView != null) {
        ...
        public void run() {
            if (mScaleView != null) {
                ...
                if ((!mIsFinished) && (mScale > 1.0D)) {
                    ...
                    f2 = mScale - (mScale - 1.0F) * ScaleScrollView.sInterpolator.getInterpolation(f1);
                    localLayoutParams = mHeaderContainer.getLayoutParams();
                    if (f2 > 1.0F) {
                        localLayoutParams.height = ((int) (f2 * mHeaderHeight));
                        mHeaderContainer.setLayoutParams(localLayoutParams);
                        if (isCustomHeaderHeight) {
                            ViewGroup.LayoutParams ScaleLayoutParams;
                            ScaleLayoutParams = mScaleView.getLayoutParams();
                            ScaleLayoutParams.height = ((int) (f2 * mHeaderHeight));
                            mScaleView.setLayoutParams(ScaleLayoutParams);
                        }
                        post(this);
                        return;
